...
| `python -m archival_pipeline.cli <目录> --translate --preview preview.json` | 翻译+档案化预览 |
| `python -m archival_pipeline.cli <目录> --execute --backup backup` | 执行+备份 |
| `python -m archival_pipeline.cli <目录> --translate --execute --backup backup` | 翻译+档案化+备份 |
| `python -m archival_pipeline.cli <目录> --flatten all` | 纯平铺（全部文件） |
| `python -m archival_pipeline.cli <目录> --execute --backup backup --flatten all` | 档案化 + 平铺 |
| `python -m archival_pipeline.cli <目录> --translate --execute --backup backup --flatten archived` | 翻译+档案化+平铺(仅已变更) |
