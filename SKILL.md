...
| `python -m archival_pipeline.cli <目录> --translate table --preview preview.json` | 映射表翻译预览 |
| `python -m archival_pipeline.cli <目录> --translate google --preview preview.json` | Google免费翻译预览 |
| `python -m archival_pipeline.cli <目录> --translate refine --step-config translator.api_key=sk-xxx --preview preview.json` | AI精修预览（角色名/基调/语序） |
| `python -m archival_pipeline.cli <目录> --execute --backup backup` | 执行+备份 |
| `python -m archival_pipeline.cli <目录> --translate google --execute --backup backup --flatten all` | Google翻译+三删+日期继承+平铺 |
| `python -m archival_pipeline.cli <目录> --translate refine --step-config translator.api_key=sk-xxx --step-config translator.tone=adult --execute --backup backup` | AI精修（成人基调）+档案化 |
| `python -m archival_pipeline.cli <目录> --translate refine --step-config translator.tone=adult --template '{date}_{name}{ext}' --preview preview.json` | AI精修+自定义模板（不要上下文） |
